...
const DiseasesSelect = ({noticia, setNoticia}) => {

    let async_component;

    const {enfermedades,obtieneEnfermedades} = useContext(FormContext);

    const [defaultDiseases,
        setdefaultDiseases] = useState([]);
    ...
    const setDiseases = (values) => {

        let new_noticia = {

            ...noticia,
            diseases: {
                active: noticia.diseases.active,
                list: values
            }
        }
    ...
    const busquedaDiseases = (inputValue) => {
        ...
        if (inputValue.length >= 3) {
            ...
            obtieneEnfermedades(inputValue);
           
    

            
            enfermedades.map((value) => {
    ...
    const loadOptions = (inputValue, callback) => {
        ...
        setTimeout(() => {
            callback(busquedaDiseases(inputValue));
        }, 1);
    };

    
    const deleteDisase = id => {
            ...
            <Row className="m-0 p-1">
                <Col xs="12" className="m-0 p-0 ">
                    ...
                    <div className="toggle-container">
                        <Toggle
                            id="Diseases_active"
                            name="Diseases_active"
                            defaultChecked={!!noticia.diseases.active}
                            onChange=
                            {e=> { setLinkActive("diseases", e.target.checked ? 1 : 0) }}/>
                        <label>Vincular enfermedades</label>

                        {noticia.diseases.active == 1
                            ? <Fragment>
                                    <Button onClick={e => deleteAllDiseases()}>{IconDelete}Borrar todo</Button>
                                    <Button
                ...
                {noticia.diseases.active == 1 && noticia.diseases.list !== null && noticia.diseases.list.length >= 1
                    ? <Col xs="12" className="m-0 p-0">
                                    ...
                                    .map((val, key) => {
                                        return <ListGroupItem className="m-0 mb-1 p-1" key={val.value}>
                                            <div className="d-flex">
                                                <div>{val.label}</div>
                                                <div onClick={e => deleteDisase(val.value)} className="float-right">{IconDelete}</div>
                                            </div>
                                        </ListGroupItem>
                                    })
}
