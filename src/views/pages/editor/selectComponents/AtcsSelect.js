...
const AtcsSelect = ({noticia, setNoticia}) => {

    let async_component;

    const {atcs,obtieneAtcs} = useContext(FormContext);

    const [defaultAtcs,
        setdefaultAtcs] = useState([]);
    ...
    const setAtcs = (values) => {

        let new_noticia = {

            ...noticia,
            atcs: {
                active: noticia.atcs.active,
                list: values
            }
        }
        ...
        if (inputValue.length >= 3) {
            ...
            atcs.map((value) => {
                ...
                if (value.nombre.toLowerCase().includes(inputValue.toLowerCase())) {
                    options.push({value: value.codigo, label: value.codigo + " - " + value.nombre});
    ...
    const loadOptions = (inputValue, callback) => {
        ...
        setTimeout(() => {
            callback(busquedaAtcs(inputValue));
        }, 1);
    };
    ...
    return (
        ...
        <Container fuid="true" className="EditorFormPage m-0 mt-1 p-0">
            ...
            <Row className="m-0 p-1">
                <Col xs="12" className="m-0 p-0 ">
                    ...
                    <div className="toggle-container">
                        <Toggle
                            id="Atcs_active"
                            name="Atcs_active"
                            defaultChecked={!!noticia.atcs.active}
                            onChange=
                            {e=> { setLinkActive("atcs", e.target.checked ? 1 : 0) }}/>
                        <label>Vincular ATCs</label>

                        {noticia.atcs.active == 1
                            ? <Fragment>
                                    <Button onClick={e => deleteAllAtcs()}>{IconDelete}Borrar todo</Button>
                                    <Button
                ...
                {noticia.atcs.active == 1 && noticia.atcs.list !== null && noticia.atcs.list.length >= 1
                    ? <Col xs="12" className="m-0 p-0">
                                    ...
                                    .map((val, key) => {
                                        return <ListGroupItem className="m-0 mb-1 p-1" key={val.value}>
                                            <div className="d-flex">
                                                <div>{val.label}</div>
                                                <div onClick={e => deleteAtc(val.value)} className="float-right">{IconDelete}</div>
                                            </div>
                                        </ListGroupItem>
                                    })
}
